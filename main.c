...
static esp_err_t root_get_handler(httpd_req_t *req)
{
    const char *html_page =
        "<!DOCTYPE html><html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Smart Basil - Plant Monitor</title><link href='https://fonts.googleapis.com/css2?family=DM+Mono:wght@400;500&family=Fraunces:ital,wght@0,300;0,600;1,300&display=swap' rel='stylesheet'><style>:root{--green-deep:#1a3a2a;--green-mid:#2d6a4f;--green-light:#52b788;--cream:#f4f0e8;--amber:#e08c3a;--red:#c0392b;--text:#1a3a2a;--text-soft:#4a7c5e;--card-bg:#fff;--border:#d8e8de}*{margin:0;padding:0;box-sizing:border-box}body{font-family:'DM Mono',monospace;background-color:var(--cream);min-height:100vh;color:var(--text);padding:28px 12px}.page-wrap{max-width:680px;margin:0 auto}header{display:flex;align-items:flex-end;gap:12px;margin-bottom:28px}.leaf-icon{width:44px;height:44px;background:var(--green-mid);border-radius:60% 5% 60% 5%;display:flex;align-items:center;justify-content:center;font-size:20px;box-shadow:2px 4px 10px rgba(45,106,79,0.25)}h1{font-family:'Fraunces',serif;font-weight:600;font-size:1.8rem;color:var(--green-deep)}h1 span{display:block;font-style:italic;font-weight:300;font-size:.55em;color:var(--text-soft);margin-top:4px}.status-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px}.card{background:var(--card-bg);border:1px solid var(--border);border-radius:12px;padding:14px 12px;position:relative;overflow:hidden}.card::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:var(--green-light);border-radius:12px 12px 0 0}.card.alert::before{background:var(--red)}.card.active::before{background:var(--green-mid)}.card-label{font-size:.68em;letter-spacing:.12em;text-transform:uppercase;color:var(--text-soft);margin-bottom:8px}.card-value{font-family:'Fraunces',serif;font-size:1.6rem;font-weight:600;color:var(--green-deep)}.detail-card{background:var(--card-bg);border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:16px}.row{display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--border);font-size:.85em}.row:last-child{border-bottom:none}.row-label{color:var(--text-soft)}.row-val{color:var(--text);font-weight:500}.bar-wrap{display:flex;align-items:center;gap:8px}.bar{width:100px;height:6px;background:var(--border);border-radius:99px;overflow:hidden}.bar-fill{height:100%;background:linear-gradient(90deg,var(--green-mid),var(--green-light));border-radius:99px;transition:width .5s}.pump-card{background:var(--card-bg);border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:16px}.pump-controls{display:flex;align-items:center;gap:12px;flex-wrap:wrap}.toggle{position:relative;width:48px;height:26px;cursor:pointer}.toggle input{opacity:0;width:0;height:0}.slider{position:absolute;inset:0;background:var(--border);border-radius:99px;transition:background .25s}.slider::after{content:'';position:absolute;width:18px;height:18px;left:4px;top:4px;background:#fff;border-radius:50%;transition:transform .25s;box-shadow:0 1px 3px rgba(0,0,0,.18)}.toggle input:checked+.slider{background:var(--green-mid)}.toggle input:checked+.slider::after{transform:translateX(22px)}.pump-pill{display:inline-flex;align-items:center;gap:7px;padding:6px 12px;border-radius:99px;font-size:.78em;font-weight:500}.pump-pill.off{background:#f0f4f1;color:var(--text-soft)}.pump-pill.on{background:#e6f4ec;color:var(--green-mid)}.pump-dot{width:8px;height:8px;border-radius:50%;background:currentColor}.override-tag{font-size:.72em;background:#fff3e0;color:var(--amber);padding:3px 9px;border-radius:99px;border:1px solid #f0c060;display:none}.timestamp{text-align:center;font-size:.72em;color:var(--text-soft);margin-top:8px}.timestamp.error{color:var(--red)}</style></head><body><div class='page-wrap'><header><div class='leaf-icon'>🌿</div><h1>Smart Basil<span>plant monitoring system</span></h1></header><div class='status-grid'><div class='card' id='card-water'><div class='card-label'>Water Level</div><div class='card-value' id='val-water'>—</div></div><div class='card' id='card-pump'><div class='card-label'>Pump</div><div class='card-value' id='val-pump'>—</div></div><div class='card'><div class='card-label'>Soil Moisture</div><div class='card-value' id='val-soil'>—</div></div><div class='card'><div class='card-label'>Temperature</div><div class='card-value' id='val-temp'>—</div></div></div><div class='pump-card'><h2>💧 Manual Pump Control</h2><p style='font-size:.82em;color:var(--text-soft);margin:8px 0 12px'>Override automatic watering. Manual control sends a POST to /api/pump.</p><div class='pump-controls'><label class='toggle'><input type='checkbox' id='pump-toggle' onchange='handlePumpToggle(this.checked)'><span class='slider'></span></label><span class='toggle-label' id='toggle-label'>Pump off</span><div class='pump-pill off' id='pump-pill'><div class='pump-dot'></div><span id='pill-text'>IDLE</span></div><span class='override-tag' id='override-tag'>MANUAL OVERRIDE</span></div></div><div class='detail-card'><h2>Sensor Details</h2><div class='row'><span class='row-label'>Soil moisture</span><div class='bar-wrap'><div class='bar'><div class='bar-fill' id='soil-bar' style='width:0%'></div></div><span class='row-val' id='detail-soil'>—</span></div></div><div class='row'><span class='row-label'>Humidity</span><div class='bar-wrap'><div class='bar'><div class='bar-fill' id='hum-bar' style='width:0%'></div></div><span class='row-val' id='detail-hum'>—</span></div></div><div class='row'><span class='row-label'>Temperature</span><span class='row-val' id='detail-temp'>—</span></div><div class='row'><span class='row-label'>Pump ON threshold</span><span class='row-val' id='detail-on-thresh'>—</span></div><div class='row'><span class='row-label'>Pump OFF threshold</span><span class='row-val' id='detail-off-thresh'>—</span></div><div class='row'><span class='row-label'>Raw ADC value</span><span class='row-val' id='detail-raw'>—</span></div></div><div class='timestamp' id='timestamp'>Waiting for data…</div></div><script>function render(d){const ts=document.getElementById('timestamp');if(!d){ts.textContent='No data — connect to ESP32 AP';ts.className='timestamp error';return}const cardW=document.getElementById('card-water'),valW=document.getElementById('val-water');if(d.water_ok){cardW.className='card active';valW.textContent='GOOD'}else{cardW.className='card alert';valW.textContent='LOW'}const cardP=document.getElementById('card-pump'),valP=document.getElementById('val-pump');if(d.pump_on){cardP.className='card active';valP.textContent='ON'}else{cardP.className='card';valP.textContent='OFF'}document.getElementById('val-soil').textContent=(d.soil_percent!==undefined)?d.soil_percent+'%':'—';document.getElementById('val-temp').textContent=(d.temp_f!==undefined)?d.temp_f.toFixed(1)+'°F':'—';document.getElementById('detail-soil').textContent=(d.soil_percent!==undefined)?d.soil_percent+'%':'—';document.getElementById('detail-hum').textContent=(d.hum_rh!==undefined)?d.hum_rh.toFixed(1)+'%':'—';document.getElementById('detail-temp').textContent=(d.temp_f!==undefined&&d.temp_c!==undefined)?d.temp_f.toFixed(1)+'°F / '+d.temp_c.toFixed(1)+'°C':'—';document.getElementById('detail-on-thresh').textContent=(d.pump_on_threshold!==undefined)?d.pump_on_threshold+'%':'—';document.getElementById('detail-off-thresh').textContent=(d.pump_off_threshold!==undefined)?d.pump_off_threshold+'%':'—';document.getElementById('detail-raw').textContent=(d.soil_raw!==undefined)?d.soil_raw:'—';document.getElementById('soil-bar').style.width=((d.soil_percent||0)+'%');document.getElementById('hum-bar').style.width=((d.hum_rh||0)+'%');const pill=document.getElementById('pump-pill'),pillText=document.getElementById('pill-text');if(d.pump_on){pill.className='pump-pill on';pillText.textContent='RUNNING'}else{pill.className='pump-pill off';pillText.textContent='IDLE'}const toggle=document.getElementById('pump-toggle');if(toggle&&document.activeElement!==toggle)toggle.checked=!!d.pump_on;document.getElementById('toggle-label').textContent=d.pump_on?'Pump on':'Pump off';ts.textContent='Last updated: '+new Date().toLocaleTimeString();ts.className='timestamp'}function handlePumpToggle(checked){fetch('/api/pump',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({on:!!checked})}).catch(()=>{});const pill=document.getElementById('pump-pill');pill.className=checked?'pump-pill on':'pump-pill off';document.getElementById('pill-text').textContent=checked?'RUNNING':'IDLE';document.getElementById('toggle-label').textContent=checked?'Pump on':'Pump off'}function fetchData(){fetch('/api/status').then(r=>{if(!r.ok)throw new Error('no data');return r.json()}).then(data=>render(data)).catch(()=>render(null))}fetchData();setInterval(fetchData,2000)</script></body></html>";
